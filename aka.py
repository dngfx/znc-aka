...
# This software was updated to work with the most recent version of ZNC
# Without shitting the bed.

# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
#  Authors: Evan (MuffinMedic), Aww (AwwCookies), dfx (dongfix)           #
...
class aka( znc.Module ):
			...
			for net in nets:
				...
				for chan in chans:
					...
					for nick in nicks.items():
						self.process_user( net.GetName(), nick[ 1 ].GetNick(), nick[ 1 ].GetIdent(),
	...
	def cmd_history( self, user ):
		...
		self.cur.execute(
			"SELECT DISTINCT nick, host FROM users WHERE network = '{0}' AND (nick GLOB '{1}' OR ident GLOB '{1}' OR host GLOB '{1}');".format(
	...
	def cmd_seen( self, user, channel ):
		if channel:
			self.cur.execute(
				"SELECT nick, ident, host, channel, message, MAX(time) FROM (SELECT * from users WHERE message IS NOT NULL) WHERE network = '{0}' AND channel = '{1}' AND (nick GLOB '{2}' OR ident GLOB '{2}' OR host GLOB '{2}');".format(
					self.GetNetwork().GetName().lower(), channel.lower(), user.lower() ) )
		else:
			self.cur.execute(
				"SELECT nick, ident, host, channel, message, MAX(time) FROM (SELECT * from users WHERE message IS NOT NULL) WHERE network = '{0}' AND (nick GLOB '{1}' OR ident GLOB '{1}' OR host GLOB '{1}');".format(
					self.GetNetwork().GetName().lower(), user.lower() ) )
		data = self.cur.fetchone()
		try:
			self.PutModule(
				"\x02{}\x02 ({}@{}) was last seen in \x02{}\x02 at \x02{}\x02 saying \"\x02{}\x02\".".format( data[ 0 ],
				                                                                                              data[ 1 ],
				                                                                                              data[ 2 ],
				                                                                                              data[ 3 ],
	...
	def cmd_users( self, user ):
		self.cur.execute(
			"SELECT DISTINCT nick, host, ident FROM users WHERE network = '{0}' AND (nick GLOB '{1}' OR ident GLOB '{1}' OR host GLOB '{1}');".format(
				self.GetNetwork().GetName().lower(), user.lower() ) )
		data = self.cur.fetchall()
		chans = set()
		for row in data:
	...
	def cmd_channels( self, users ):
		...
		for user in users:
			chans = [ ]
			self.cur.execute(
				"SELECT DISTINCT channel FROM users WHERE network = '{0}' AND (nick GLOB '{1}' OR ident GLOB '{1}' OR host GLOB '{1}');".format(
					self.GetNetwork().GetName().lower(), user.lower() ) )
			data = self.cur.fetchall()
			for row in data:
				chans.append( row[ 0 ] )
		...
		self.PutModule( "Common \x02channels\x02 for \x02{}:\x02 {}".format( ', '.join( users ),
		                                                                     ', '.join( sorted( shared_chans ) ) ) )
	...
	def cmd_users( self, channels ):
		...
		for channel in channels:
			...
			self.cur.execute(
				"SELECT DISTINCT nick, ident, host FROM users WHERE network = '{}' AND channel = '{}';".format(
					self.GetNetwork().GetName().lower(), channel.lower() ) )
			data = self.cur.fetchall()
			for row in data:
				nicks.append( row[ 0 ] );
				idents.append( row[ 1 ] );
		...
		self.PutModule( "Common \x02nicks\x02 for \x02{}:\x02 {}".format( ', '.join( channels ),
		                                                                  ', '.join( sorted( shared_nicks ) ) ) )
		self.PutModule( "Common \x02idents\x02 for \x02{}:\x02 {}".format( ', '.join( channels ),
		                                                                   ', '.join( sorted( shared_idents ) ) ) )
		self.PutModule( "Common \x02hosts\x02 for \x02{}:\x02 {}".format( ', '.join( channels ),
		                                                                  ', '.join( sorted( shared_hosts ) ) ) )
	...
	def cmd_geo( self, user ):
		...
		try:
			...
			if loc_json[ "status" ] != "fail":
				...
				self.PutModule(
					"{} is located in \x02{}, {}, {}\x02 ({}, {}) / Timezone: {} / Proxy: {} / Mobile: {} / IP: {} / rDNS: {}".format(
				...
				self.PutModule(
					"\x02\x034Unable to geolocate\x03\x02 user \x02{}\x02. (Reason: {})".format( user.lower(), loc_json[
						"message" ] ) )
		except:
	...
	def cmd_stats( self ):
		self.cur.execute(
			"SELECT COUNT(DISTINCT nick), COUNT(DISTINCT ident), COUNT(DISTINCT host), COUNT(DISTINCT channel), COUNT(*) FROM users WHERE network = '{0}';".format(
	...
	def cmd_who( self, scope ):
		...
		self.PutModule(
			"{} WHO updates triggered. Please wait several minutes for ZNC to receive the updated data from the IRC server(s) and then run \x02process\x02 to add these updates to the database".format(
				scope ) )
	...
	def db_setup( self ):
		...
		self.cur.execute(
			"create table if not exists users (id INTEGER PRIMARY KEY, network TEXT, nick TEXT, ident TEXT, host TEXT, channel TEXT, message TEXT, time INTEGER, UNIQUE(network, nick, ident, host, channel));" )
		self.conn.commit()
		...
		for net in nets:
			...
			if os.path.exists( path ):
				...
				for row in data:
					if row[ 5 ]:
						self.cur.execute(
							"INSERT OR IGNORE INTO users (network, nick, ident, host, channel, message, time) VALUES (LOWER(?), LOWER(?), LOWER(?), LOWER(?), LOWER(?), LOWER(?), LOWER(?));",
							(net.GetName(), row[ 0 ], row[ 1 ], row[ 2 ], row[ 3 ], row[ 4 ], str( time.mktime( (
								                                                                                    datetime.datetime.strptime(
									                                                                                    row[
										                                                                                    5 ].partition(
										                                                                                    '.' )[
										                                                                                    0 ],
									                                                                                    '%Y-%m-%d %H:%M:%S' )).timetuple() ) ).partition(
								'.' )[ 0 ]) )
					elif row[ 6 ]:
						self.cur.execute(
							"INSERT OR IGNORE INTO users (network, nick, ident, host, channel, message, time) VALUES (LOWER(?), LOWER(?), LOWER(?), LOWER(?), LOWER(?), LOWER(?), LOWER(?);",
						...
						self.cur.execute(
							"INSERT OR IGNORE INTO users (network, nick, ident, host, channel, message) VALUES (LOWER(?), LOWER(?), LOWER(?), LOWER(?), LOWER(?), LOWER(?));",
							(net.GetName(), row[ 0 ], row[ 1 ], row[ 2 ], row[ 3 ], row[ 4 ]) )
				self.conn.commit()
				os.rename( path,
				           self.GetUser().GetUserPath() + "/networks/" + net.GetName() + "/moddata/aka/aka." + net.GetName() + ".db.old" )

	def OnModCommand( self, command ):
		command = command.lower()
		cmds = [ "all", "history", "users", "channels", "sharedchans", "sharedusers", "seen", "geo", "process", "who",
		         "rawquery", "stats", "about", "help", "migrate" ]
		if command.split()[ 0 ] in cmds:
			if command.split()[ 0 ] == "all":
				...
				try:
					...
					self.PutModule( "You must specify a user." )
			elif command.split()[ 0 ] == "users" or command.split()[ 0 ] == "channels" or command.split()[
				0 ] == "sharedchans" or command.split()[ 0 ] == "sharedusers":
	...
	def cmd_cmdlist( self ):
		self.PutModule("COMMAND: all <user> -- show all information about a user")
	...
	def cmd_help( self ):
		...
		help.SetCell( "Description", "Update userdata on all users in the scope" )
		help.AddRow()
		help.SetCell( "Command", "process" )
		help.SetCell( "Arguments", "<scope>" )
		help.SetCell( "Description", "Add all current users in the scope to the database" )
		help.AddRow()
		...
		help.SetCell( "Description",
		              "<user> supports * and ? GLOB wildcard syntax" )

		self.__output_table(help)
